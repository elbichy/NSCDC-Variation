...
class ConmessSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $collection = (new FastExcel)->import(storage_path('conmess_salary_chart.csv'));
        foreach ($collection as $row) {
            // dd((float)$row['net_pay'];
            $step = Conmess::create([
                'rank_id' => $row['rank_id'] == '' ? null : (float)$row['rank_id'],
                'conmess_gl' => $row['conmess_gl'] == '' ? null : (float)$row['conmess_gl'],
                'conmess_step' => $row['conmess_step'] == '' ? null : (float)$row['conmess_step'],
